...
export const WorkExperienceSchema = z.object({
  ...
  description: z.string().optional().default(""), // legacy compat — kept for old Redis data
});

export const EducationSchema = z.object({
  type: z
    .enum(["degree", "12th", "10th", "diploma", "other"])
    .default("degree"),
...
export const SectionTogglesSchema = z.object({
  ...
  buildSection: z
    .object({ showInPortfolio: z.boolean().default(true) })
    .default({}),
});
...
export function getSectionVisibility(portfolio: Portfolio) {
  ...
  return {
    ...
    showEducationInPortfolio: s?.education?.showInPortfolio ?? true,
    showCertsInPortfolio: s?.certifications?.showInPortfolio ?? true,
    showProjectsInResume: s?.projects?.showInResume ?? true,
    showWorkInResume: s?.workExperience?.showInResume ?? true,
    showEducationInResume: s?.education?.showInResume ?? true,
    showCertsInResume: s?.certifications?.showInResume ?? true,
  };
}
