      ...
      <form onSubmit={handleSubmit} className="form" noValidate>
        ...
        <div className="input-div">
          ...
          <input
            ...
            onChange={(e) =>
              set("yearsOfExperience", parseInt(e.target.value, 10) || 0)
            }
          />
        </div>
        <div className="input-div">
            ...
            <div className="input-div">
              ...
              <div className="flex gap-3 mt-3">
                <select
                  ...
                  onChange={(e) => updateWork(i, "startMonth", e.target.value)}
            ...
            <div className="input-div">
              <div className="flex items-center justify-between mt-3">
                ...
                <button
                  type="button"
                  role="switch"
                  aria-checked={work.isCurrentJob}
                  aria-label="Toggle current job"
                  onClick={() =>
                    updateWork(i, "isCurrentJob", !work.isCurrentJob)
                  }
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${work.isCurrentJob ? "bg-zinc-800 dark:bg-zinc-200" : "bg-zinc-300 dark:bg-zinc-600"}`}
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white dark:bg-black shadow transition-transform ${work.isCurrentJob ? "translate-x-6" : "translate-x-1"}`}
                  />
                </button>
              </div>
            </div>

            {/* End date — hidden when current job */}
            {!work.isCurrentJob && (
              <div className="input-div">
                ...
                <div className="flex gap-3 mt-3">
                  <select
                    className="input flex-1"
                    value={work.endMonth}
                    onChange={(e) => updateWork(i, "endMonth", e.target.value)}
        ...
        <p className="text-sm text-zinc-500 mb-2">
          Toggle which sections appear on your live portfolio.
        </p>
        ...
        {[
          ...
          <div
            key={key}
            className="flex items-center justify-between py-3 border-b border-zinc-100 dark:border-zinc-800"
          >
            <span className="text-sm font-medium">{label}</span>
            <button
              type="button"
              onClick={() =>
                set(key as keyof PortfolioFormData, !(form as any)[key])
              }
            ...
            >
              <span
                className={`inline-block h-4 w-4 transform rounded-full bg-white dark:bg-black shadow transition-transform ${(form as any)[key] ? "translate-x-6" : "translate-x-1"}`}
              />
            </button>
          </div>
        ))}
