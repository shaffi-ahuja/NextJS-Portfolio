...
const emptyWork     = () => ({ company:'', title:'', startMonth:'', startYear:'', endMonth:'', endYear:'', isCurrentJob:false, description:'' });
const emptyEdu      = () => ({ type: 'degree' as const, institutionName:'', fieldOfStudy:'', startYear:'', endYear:'', grade:'', location:'' });
const emptyCert     = () => ({ name:'', organization:'', date:'', credentialUrl:'' });
...
function Toggle({ checked, onChange, label }: { checked: boolean; onChange: () => void; label: string }) {
  return (
    ...
    >
      <span className={`inline-block h-4 w-4 transform rounded-full bg-white dark:bg-black shadow transition-transform ${checked ? 'translate-x-6' : 'translate-x-1'}`} />
    </button>
  );
}
...
function SkillMultiSelect({ selected, onChange }: { selected: string[]; onChange: (s: string[]) => void }) {
...
export default function UserForm() {
  ...
  const [form, setForm] = useState<PortfolioFormData>({
    ...
    gender: 'male', experienceSummary: '', locatedAt: '',
  ...
  const toggleSection = (
    ...
    setForm(f => ({
      ...
      sections: {
        ...f.sections,
        [key]: { ...(f.sections[key] as any), [subKey]: !(f.sections[key] as any)[subKey] },
      },
    }));
  };
  ...
  const updateProject = (i: number, field: string, value: unknown) => {
    const updated = [...form.projects]; updated[i] = { ...updated[i], [field]: value };
  ...
  const updateWork = (i: number, field: string, value: string | boolean) => {
    const updated = [...form.workExperiences]; updated[i] = { ...updated[i], [field]: value };
  ...
  const updateEdu = (i: number, field: string, value: string) => {
    const updated = [...form.education]; updated[i] = { ...updated[i], [field]: value };
  ...
  const updateCert = (i: number, field: string, value: string) => {
    const updated = [...form.certifications]; updated[i] = { ...updated[i], [field]: value };
    setForm(f => ({ ...f, certifications: updated }));
  };
  const addCert    = () => setForm(f => ({ ...f, certifications: [...f.certifications, emptyCert()] }));
  ...
  const validate = useCallback((): boolean => {
    ...
    form.education.forEach((edu, i) => {
      if (!edu.institutionName.trim()) e[`edu_name_${i}`] = 'Institution name is required';
    });
    form.certifications.forEach((cert, i) => {
      if (!cert.name.trim())         e[`cert_name_${i}`] = 'Certification name is required';
      if (!cert.organization.trim()) e[`cert_org_${i}`]  = 'Organization is required';
    });
    setErrors(e);
  ...
  return (
    <div className="user-form-container pb-20">
      ...
      <form onSubmit={handleSubmit} className="form" noValidate>
        ...
        <div className="input-div items-center">
          ...
          <div className="flex items-center gap-5 mt-3">
            ...
            <div className="flex flex-col gap-2">
              <label htmlFor="profileImageInput" className="cursor-pointer border border-zinc-400 px-4 py-2 rounded-md text-sm hover:bg-zinc-100 dark:hover:bg-zinc-800 inline-block text-center">
                {form.profileImage ? 'Change photo' : 'Upload photo'}
              </label>
              {form.profileImage && (
                <button type="button" onClick={() => set('profileImage', '')} className="text-red-500 text-sm hover:underline text-left">Remove</button>
              )}
              <p className="text-xs text-zinc-500">JPG, PNG or WEBP · Max 2MB</p>
            </div>
          </div>
          <input id="profileImageInput" type="file" accept="image/jpeg,image/png,image/webp" className="hidden"
            onChange={(e) => {
              const file = e.target.files?.[0];
              if (!file) return;
              if (file.size > 2 * 1024 * 1024) { setErrors(err => ({ ...err, profileImage: 'Image must be under 2MB' })); return; }
        ...
        <div className="input-div">
          <label htmlFor="phone">Phone number <span className="text-zinc-500 font-normal text-sm">(optional — shown in resume PDF)</span></label>
          <input id="phone" type="tel" className="input" value={form.phone} onChange={e => set('phone', e.target.value)} placeholder="+91-9876543210" />
        </div>
        <div className="input-div">
        ...
        {form.projects.map((project, i) => (
          <div key={i} className="border border-zinc-300 dark:border-zinc-700 rounded-md p-4 mt-4">
            <div className="flex justify-between items-center mb-2">
              ...
              {form.projects.length > 1 && <button type="button" onClick={() => removeProject(i)} className="text-red-500 text-sm hover:underline">Remove</button>}
            </div>
            <div className="input-div">
        ...
        {form.workExperiences.map((work, i) => (
          <div key={i} className="border border-zinc-300 dark:border-zinc-700 rounded-md p-4 mt-4">
            <div className="flex justify-between items-center mb-2">
              ...
              {form.workExperiences.length > 1 && <button type="button" onClick={() => removeWork(i)} className="text-red-500 text-sm hover:underline">Remove</button>}
            </div>
            <div className="input-div">
              <label>Company *</label>
            ...
            <div className="input-div">
              ...
              <div className="flex gap-3 mt-3">
                <select className="input flex-1" value={work.startMonth} onChange={e => updateWork(i, 'startMonth', e.target.value)} aria-label="Start month">
                  <option value="">Month</option>
                  {MONTHS.map(m => <option key={m} value={m}>{m}</option>)}
                </select>
                <input className="input flex-1" value={work.startYear} onChange={e => updateWork(i, 'startYear', e.target.value)} placeholder="2022" aria-label="Start year" data-error={!!errors[`work_start_${i}`]} />
              </div>
            ...
            <div className="input-div">
              <div className="flex items-center justify-between mt-3">
                ...
                <Toggle checked={work.isCurrentJob} onChange={() => updateWork(i, 'isCurrentJob', !work.isCurrentJob)} label="Toggle current job" />
              </div>
            </div>
            {!work.isCurrentJob && (
              <div className="input-div">
                ...
                <div className="flex gap-3 mt-3">
                  <select className="input flex-1" value={work.endMonth} onChange={e => updateWork(i, 'endMonth', e.target.value)} aria-label="End month">
                    <option value="">Month</option>
                    {MONTHS.map(m => <option key={m} value={m}>{m}</option>)}
                  </select>
                  <input className="input flex-1" value={work.endYear} onChange={e => updateWork(i, 'endYear', e.target.value)} placeholder="2024" aria-label="End year" />
                </div>
              </div>
            )}
            <div className="input-div">
        ...
        {form.education.map((edu, i) => (
          <div key={i} className="border border-zinc-300 dark:border-zinc-700 rounded-md p-4 mt-4">
            <div className="flex justify-between items-center mb-2">
              ...
              <button type="button" onClick={() => removeEdu(i)} className="text-red-500 text-sm hover:underline">Remove</button>
            </div>
            <div className="input-div">
              <label>Type</label>
              <select className="input" value={edu.type} onChange={e => updateEdu(i, 'type', e.target.value)}>
            ...
            <div className="input-div">
              ...
              <input className="input" value={edu.institutionName} onChange={e => updateEdu(i, 'institutionName', e.target.value)} placeholder="IIT Delhi / Your School Name" data-error={!!errors[`edu_name_${i}`]} />
              <FieldError msg={errors[`edu_name_${i}`]} />
            </div>
            <div className="input-div">
              <label>Field of study / Degree name</label>
              <input className="input" value={edu.fieldOfStudy} onChange={e => updateEdu(i, 'fieldOfStudy', e.target.value)} placeholder="Bachelor of Technology — Computer Science" />
            </div>
            <div className="input-div">
              <label>Years</label>
              <div className="flex gap-3 mt-3">
                <input className="input flex-1" value={edu.startYear} onChange={e => updateEdu(i, 'startYear', e.target.value)} placeholder="2015" aria-label="Start year" />
                <input className="input flex-1" value={edu.endYear} onChange={e => updateEdu(i, 'endYear', e.target.value)} placeholder="2019" aria-label="End year" />
              </div>
            </div>
            <div className="input-div">
              <label>Grade / CGPA / Percentage</label>
              <input className="input" value={edu.grade} onChange={e => updateEdu(i, 'grade', e.target.value)} placeholder="8.5 CGPA / 85%" />
            </div>
            <div className="input-div">
              <label>Location</label>
              <input className="input" value={edu.location} onChange={e => updateEdu(i, 'location', e.target.value)} placeholder="New Delhi, India" />
            </div>
          </div>
        ))}
        <button type="button" onClick={addEdu} className="mt-4 text-sm border border-zinc-400 px-4 py-2 rounded-md hover:bg-zinc-100 dark:hover:bg-zinc-800">
          + Add education
        </button>
        ...
        {form.certifications.map((cert, i) => (
          <div key={i} className="border border-zinc-300 dark:border-zinc-700 rounded-md p-4 mt-4">
            <div className="flex justify-between items-center mb-2">
              ...
              <button type="button" onClick={() => removeCert(i)} className="text-red-500 text-sm hover:underline">Remove</button>
            </div>
            <div className="input-div">
              <label>Certification name *</label>
              <input className="input" value={cert.name} onChange={e => updateCert(i, 'name', e.target.value)} placeholder="React — The Complete Guide" data-error={!!errors[`cert_name_${i}`]} />
              <FieldError msg={errors[`cert_name_${i}`]} />
            </div>
            <div className="input-div">
              <label>Issuing organization *</label>
              <input className="input" value={cert.organization} onChange={e => updateCert(i, 'organization', e.target.value)} placeholder="Udemy / Coursera / Google" data-error={!!errors[`cert_org_${i}`]} />
              <FieldError msg={errors[`cert_org_${i}`]} />
            </div>
            <div className="input-div">
              <label>Date</label>
              <input className="input" value={cert.date} onChange={e => updateCert(i, 'date', e.target.value)} placeholder="2023" />
            </div>
            <div className="input-div">
              <label>Credential URL</label>
              <input className="input" value={cert.credentialUrl} onChange={e => updateCert(i, 'credentialUrl', e.target.value)} placeholder="https://coursera.org/verify/..." />
            </div>
          </div>
        ))}
        <button type="button" onClick={addCert} className="mt-4 text-sm border border-zinc-400 px-4 py-2 rounded-md hover:bg-zinc-100 dark:hover:bg-zinc-800">
          + Add certification
        </button>
        ...
        <p className="text-sm text-zinc-500 mb-4">Control where each section appears — resume PDF, portfolio website, or both.</p>
        ...
        <div className="border border-zinc-200 dark:border-zinc-700 rounded-md overflow-hidden">
          ...
          {([
            ...
            { key: 'workExperience', label: 'Work Experience',  hasResume: true },
            { key: 'education',      label: 'Education',        hasResume: true },
            { key: 'certifications', label: 'Certifications',   hasResume: true },
            { key: 'contact',        label: 'Contact Form',     hasResume: false },
          ] as const).map(({ key, label, hasResume }) => (
            <div key={key} className="grid grid-cols-3 items-center px-4 py-3 border-b border-zinc-100 dark:border-zinc-800 last:border-0">
